...
export class AutoComplete extends Component {
    ...
    static getCoinBySelectedText = (suggestions, text) => {
        ...
        const slugs = suggestions.map(c => c.split(',')[1].toLowerCase());
    ...
    constructor(props) {
        ...
        if (props.defaultCoin) {
            const [name, slug] = AutoComplete.getCoinBySelectedText(props.suggestions, props.defaultCoin);
    ...
    return <Query query={GET_COINS}>
        {({loading, error, data, fetchMore}) => {
            ...
            return <div className="autocomplete-wrapper">
                <AutoComplete suggestions={data.getCoins.map(c => c.tokens)} {...props}/>
            </div>
        }
        }
    </Query>
